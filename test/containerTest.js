...
const CANNON = require( '../src/cannon' );
    ...
    function updatePhysics( world ){
        // Step world
        var timeStep = 1 / 60/*settings.stepFrequency*/;
        ...
        world.step(timeStep, timeSinceLastCall, 4);
	...
	const spheres = [];
	createContainer( world, nx, ny, 4 );

	for( var time = 0; time < 4.0; time += 1/60 ) {
		//console.log( "Sphere time: -----------------------", time.toFixed(4) );
		updatePhysics( world );
		
        for( var i = 0; i < spheres.length; i++ ) {
        	const s = spheres[i];
			if( s.position.x > 4.1 || s.position.x < -4.1 )
                console.log( "FailX : Sphere position is?", s.position );
			else if( s.position.y > 4.1 || s.position.y < -4.1 )
                console.log( "FailY : Sphere position is?", s.position );
        }
        ...
        for( var i = 0; i < spheres.length; i++ ) {
	        	const s = spheres[i];
			if( ( s.position.x > 4.1 || s.position.x < -4.1 )
			 	|| ( s.position.y > 4.1 || s.position.y < -4.1 ) ) {
            console.log( "Sphere position is?", s.position );
			}
        }
	

      function createContainer(world,nx,ny,nz){
