...
module.exports = {
    creation : function(test) {
        ...
        var q = new Quaternion().set(1, 2, 3, 4);
...
//        test.equal(q.w, 4, "Creating should set the fourth parameter to the w value");
...
//        test.equal(q.w,  4, ".conjugate() should not touch w");
    ...
    inverse : function(test) {
        ...
        test.equal(q.x, -1, ".inverse() should negate x and divide by length^2");
        test.equal(q.y, -2, ".inverse() should negate y and divide by length^2");
        test.equal(q.z, -3, ".inverse() should negate z and divide by length^2");
